...
//cscs void CMSB::OverheadsBench::runMicroBench (CMSB::TimeSyncInfo* syncInfo) {
void CMSB::OverheadsBench::runMicroBench () {
...
/*
    // First run the warmup runs - no need to measure times
...
/*
    ...
    //cscs syncInfo->_esttime = max_warmup_time;
*/    

    //cscs double run_times[NUM_ITERS_ROUND];
    double mem_consump[NUM_ITERS_ROUND];
    //cscs double max_run_times[NUM_ITERS_TOTAL];
...
*/
            int error_count = 0;
            //cscs CMSB::sync_init_stage2 (syncInfo);
            for (int i = 0; i < NUM_ITERS_ROUND; i++) {
                    double err = 0;
                    //cscs errors[i] = CMSB::nbcb_sync (syncInfo);

                    mem_before = 0.0;
                    CMSB::MemEstimator::startLocalPeakMemMeasurement ();
                    //cscs start_time = CMSB::elg_pform_wtime ();
                    num_internal_iters = runOverheadFunc ();
                    //cscs end_time = CMSB::elg_pform_wtime ();
                    mem_after = (double)CMSB::MemEstimator::getLocalPeakMemConsumption ();
                    cleanupOverheadFunc ();

                    //cscs run_times[i] = (end_time - start_time) * 1e6;	// Convert to usec
                    mem_consump[i] = (mem_before > mem_after) ? 0.0 : (mem_after - mem_before);
                    //cscs run_times[i] /= num_internal_iters;
                    mem_consump[i] /= num_internal_iters;
                    mem_consump[i] = (mem_before > mem_after) ? (mem_before - mem_after) : (mem_after - mem_before);

                    std::cout << "dupm "<< _myRank 
                        << " " << i 
                        << " " << std::setprecision(1) << std::fixed << mem_consump[i] << std::endl; 
/*
            if (_myRank == 0) {
            std::cout << mpi_collective_name 
                << " rk=" << _myRank 
                << " i=" << i 
                << ": mem = " << std::setprecision(1) << std::fixed << mem_consump[i] << std::endl;
                }
*/
            }

            total_num_valid_runs += 1;
...
void CMSB::createOverheadsMicroBenches (std::vector<MicroBench*>& benchmarks) {
	...
	//cscs benchmarks.push_back (new CMSB::CommcreateBench ());
	benchmarks.push_back (new CMSB::CommdupBench    ());
	//cscs benchmarks.push_back (new CMSB::WincreateBench  ());
        //cscs benchmarks.push_back (new CMSB::CartcreateBench ());
}
